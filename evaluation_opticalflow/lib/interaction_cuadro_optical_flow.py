...
class OpticalFlowSimulator1(object):
    ...
    """
    def compute_opticalflow_seq(self,Id,obs_traj,neighbors,width_bound,height_bound):
        ...
        for i in range(sequence_length):
            ...
            width_low    = x_current-width_bound/2.0
            width_high   = x_current+width_bound/2.0
            height_low   = y_current-height_bound/2.0
            height_high  = y_current+height_bound/2.0
            ...
            optical_flow[i,:], visible_neighbors[i,:,:] = self.get_flow_in_cone(obs_traj[i],direcciones[i],v_obser,p_veci,vel_p_veci)
        return optical_flow, visible_neighbors

    # Main function for optical flow computation
    def compute_opticalflow_batch_with_neighborhood(self,neighbors_batch, idx, obs_traj, obs_len,lim):
        """
        Receives:
        ...
        width_bound  = width/lim[4]
        height_bound = height/lim[4]
        ...
        for batch_idx, neighbors_descriptor in enumerate(neighbors_batch):
            ...
            vec_flow[batch_idx,:,:],vis_neigh[batch_idx,:,:,:] =  self.compute_opticalflow_seq_with_neighborhood(person_id, obs_traj[batch_idx],neighbors_descriptor,width_bound, height_bound)
        #print(self.maxi)
