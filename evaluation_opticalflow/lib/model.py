...
class TrajectoryEncoder(layers.Layer):
    ...
    def call(self, traj_inputs, training=None):
        ...
        y = self.lstm(x, training=(training or self.is_mc_dropout))
        print(y)
        return y
...
class SocialEncoder(layers.Layer):
    def __init__(self, config):
        ...
        self.traj_social_emb_enc = tf.keras.layers.Dense(config.emb_size,  # config.emb_size = tamaño del lote
                                                         # config.activacion = funcion de activavion usada
                                                         activation=config.activation_func,
...
class FocalAttention(layers.Layer):
    def __init__(self, config, M):
        ...
        # convierte las capas a una matriz de dos dimenciones
        self.flatten = tf.keras.layers.Flatten()
        # esta capa ya no tiene parametros que aprender solo reforma los datos
...
class TrajectoryAndContextEncoder(tf.keras.Model):
    def __init__(self, config):
        super(TrajectoryAndContextEncoder, self).__init__(
            name="trajectory_context_encoder")
        ...
        self.input_layer_traj = layers.Input(
            obs_shape, name="observed_trajectory")
        # Encoding: Positions
        ...
        if (self.add_attention and self.add_social):
            ...
            self.input_layer_social = layers.Input(
                soc_shape, name="social_features")
            # Encoding: Social interactions
            self.soc_enc = SocialEncoder(config)
            # Get output layer now with `call` method
            self.out = self.call(
                [self.input_layer_traj, self.input_layer_social])
        else:
            # Get output layer now with `call` method
            self.out = self.call([self.input_layer_traj])
        # Call init again. This is a workaround for being able to use summary()
        super(TrajectoryAndContextEncoder, self).__init__(
            inputs=tf.cond(self.add_attention and self.add_social, lambda: [
                           self.input_layer_traj, self.input_layer_social], lambda: [self.input_layer_traj]),
            outputs=self.out)
...
class TrajectoryDecoderInitializer(tf.keras.Model):
    def __init__(self, config):
        super(TrajectoryDecoderInitializer, self).__init__(
            name="trajectory_decoder_initializer")
        self.add_social = config.add_social
    ...
    def call(self, encoders_states, training=None):
        ...
        for i in range(self.output_var_dirs):
            ...
            decoder_init_dh = self.traj_enc_h_to_dec_h[i](
                traj_encoder_states[0])
            decoder_init_dc = self.traj_enc_c_to_dec_c[i](
                traj_encoder_states[1])
            if self.add_social:
                # Map the social features hidden states to variations of the initializer state
                decoder_init_dh = decoder_init_dh + \
                    self.traj_soc_h_to_dec_h[i](soc_encoder_states[0])
                decoder_init_dc = decoder_init_dc + \
                    self.traj_soc_c_to_dec_c[i](soc_encoder_states[1])
...
class TrajectoryDecoder(tf.keras.Model):
    def __init__(self, config):
        ...
        # RNN layer
        self.recurrentLayer = tf.keras.layers.RNN(
            self.dec_cell_traj, return_sequences=True, return_state=True)
        self.M = 1
        if (self.add_attention and self.add_social):
            self.M = self.M+1
        ...
        self.dropout = tf.keras.layers.Dropout(
            config.dropout_rate, name="dropout_decoder_h")
        ...
        self.input_layer_hid1 = layers.Input(
            enc_last_state_shape, name="initial_state_h")
        self.input_layer_hid2 = layers.Input(
            enc_last_state_shape, name="initial_state_c")
        ...
        self.out = self.call((self.input_layer_pos, (self.input_layer_hid1,
                             self.input_layer_hid2), self.input_layer_ctxt))
        # Call init again. This is a workaround for being able to use summary
        super(TrajectoryDecoder, self).__init__(
            inputs=[self.input_layer_pos, self.input_layer_hid1,
                    self.input_layer_hid2, self.input_layer_ctxt],
            outputs=self.out)
    ...
    def call(self, inputs, training=None):
        ...
        if self.add_attention:
            ...
            augmented_inputs = tf.concat(
                [decoder_inputs_emb, attention], axis=2)
        else:
            Wft = None
        ...
        outputs = self.recurrentLayer(
            augmented_inputs, initial_state=last_states, training=(training or self.is_mc_dropout))
        # Last h,c states
...
class FullTrajectoryClassifier(tf.keras.Model):
    def __init__(self, config):
        super(FullTrajectoryClassifier, self).__init__(
            name="full_trajectory_classification")
        self.is_mc_dropout = config.is_mc_dropout
        self.output_var_dirs = config.output_var_dirs
        ...
        self.input_observed = keras.Input(
            shape=input_observed_shape, name="observed_trajectory_h")
        # Inputs: overall displacement along the second part of the trajectory
        self.input_final = keras.Input(
            shape=input_final_shape, name="final_displacement")
        self.dense_layer_observed = tf.keras.layers.Dense(
            64, activation="relu", name="observed_dense")
        self.dense_layer_final = tf.keras.layers.Dense(
            64, activation="relu", name="final_dense")
        self.classification_layer = layers.Dense(
            self.output_samples, activation="softmax", name="classication")
        # Get output layer now with `call` method
...
class ObservedTrajectoryClassifier(tf.keras.Model):
    def __init__(self, config):
        super(ObservedTrajectoryClassifier, self).__init__(
            name="observed_trajectory_classification")
        self.is_mc_dropout = config.is_mc_dropout
        self.output_var_dirs = config.output_var_dirs
        self.output_samples = 2*config.output_var_dirs+1
        input_observed_shape = (config.enc_hidden_size)
...
class TrajectoryEncoderDecoder():
    ...
    def batch_step(self, batch_inputs, batch_targets, metrics, training=True):
        ...
        variables = self.enc.trainable_weights + \
            self.enctodec.trainable_weights + self.dec.trainable_weights
        ...
        with tf.GradientTape() as g:
            ...
            if self.add_social:
                traj_last_states, soc_last_states, context = self.enc(
                    batch_inputs, training=training)
                traj_cur_states_set = self.enctodec(
                    [traj_last_states[0], soc_last_states])
            else:
                traj_last_states, context = self.enc(
                    batch_inputs, training=training)
            ...
            for k in range(self.output_samples):
                ...
                for t in range(0, batch_targets.shape[1]):
                    ...
                    t_pred, dec_states, __ = self.dec(
                        [dec_input, traj_cur_states, context], training=training)
                    t_target = tf.expand_dims(batch_targets[:, t], 1)
                    # Loss
                    loss_values += (batch_targets.shape[1]-t) * \
                        self.loss_fn_local(t_target, t_pred)
                    if training == True:
            ...
            losses_at_min = tf.gather_nd(losses, tf.stack(
                [range(losses.shape[0]), closest_samples], axis=1))
    ...
    def batch_step_classifier(self, batch_inputs, batch_targets, metrics, training=True):
        ...
        with tf.GradientTape() as g:
            ...
            #########################################################################################
            # Losses are accumulated here
            metrics['obs_classif_sca'].update_state(
                closest_samples, obs_classif_logits)
            loss_value += 0.005 * tf.reduce_sum(tf.keras.losses.kullback_leibler_divergence(
                softmax_samples, obs_classif_logits))/losses.shape[0]
            # TODO: tune this value in a more principled way?
            # L2 weight decay
            loss_value += tf.add_n([tf.nn.l2_loss(v) for v in variables
                                    if 'bias' not in v.name]) * 0.0008
    ...
    def predict(self, batch_inputs, n_steps):
        ...
        for k in range(self.output_samples):
            ...
            for t in range(0, n_steps):
                # ------------------------ xy decoder--------------------------------------
                # Passing enc_output to the decoder
                t_pred, dec_states, wft = self.dec(
                    [dec_input, traj_cur_states, context], training=False)
                # Next input is the last predicted position
                dec_input = t_pred
    ...
    def predict_mcdropout(self, batch_inputs, n_steps, mc_samples=1):
        ...
        for i in range(mc_samples):
            traj_pred_set, att_weights_pred_set = self.predict(
                batch_inputs, n_steps)
