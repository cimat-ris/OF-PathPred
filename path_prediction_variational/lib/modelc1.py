...
##        lr_schedule = tf.keras.optimizers.schedules.ExponentialDecay(  ## xavi
##                config.initial_lr,
##                decay_steps=100000,
##                decay_rate=0.96,
##                staircase=True)
...
##        self.optimizer = tf.keras.optimizers.Adadelta(learning_rate=lr_schedule)
        print("lr0 model: ", config.initial_lr)
        self.optimizer = tf.keras.optimizers.SGD(learning_rate = config.initial_lr) # xavi
        ##self.optimizer = tf.keras.optimizers.SGD(learning_rate = lr_schedule) # xavi
    ...
    def batch_step(self, batch_inputs, batch_targets, metrics, training=True, loss_noise=0.0):
        ...
        if training==True:
            ...
            #print("self.dec.trainable_variables: ")
            #print(self.dec.trainable_variables[0])
            #print(self.dec.trainable_variables[1])
            #print(self.dec.trainable_variables[2])
            #print(self.dec.trainable_variables[3])
            #print(self.dec.trainable_variables[4])
            #print(self.dec.trainable_variables[5])
            #print(self.dec.trainable_variables[6])
        #########################################################################################
    ...
    def predict(self, batch_inputs, n_steps):
        traj_obs_inputs = batch_inputs[0]
        print("traj_obs_inputs: " ,traj_obs_inputs.shape)
