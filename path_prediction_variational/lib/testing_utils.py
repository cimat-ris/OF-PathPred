...
def evaluation_minadefde(model,test_data,config):
    ...
    num_batches_per_epoch= len(list(test_data)) #.cardinality().numpy()
...
#-------------------------------------------------------
from scipy.stats import gaussian_kde # xavi

def evaluation_minadefde_nll(model, test_data, config, weights):
    l2dis = []
    kde_ll = []
    kde_ll2 = []
    t_kde_ll = []
    ...
    for batch in tqdm(test_data,ascii = True):
        ...
        # Predict for all the samples of diferent cycles
        pred_out, __ = model.predict_cSGMCMC(batch_inputs,batch_targets.shape[1],weights[0][1])
        for cycle, weight in weights[1:]:
            # pred_out,__ = model.predict(batch_inputs,batch_targets.shape[1])
            pred_out_next, __ = model.predict_cSGMCMC(batch_inputs,batch_targets.shape[1],weight)
            pred_out = np.concatenate([pred_out,pred_out_next],axis=1)
        
        d = []
        # For all the trajectories in the batch
        for i, (obs_traj_gt, pred_traj_gt) in enumerate(zip(batch["obs_traj"], batch["pred_traj"])): # recorremos cada trayectoria
            normin = 999999999999.0 # 1000.0
            diffmin= None
            for k in range(len(weights)): # recorremos las posibles trayectorias predichas para una trayectoria observada
                # Conserve the x,y coordinates of the kth trajectory
                this_pred_out     = pred_out[i,k,:, :2] #[pred,2]
                ...
                # update the pred_out to absolute position
                pred_out[i,k,:, :2] = this_pred_out_abs
            # Calculate NLL
            ###nll , timestep_nll = compute_kde_nll(pred_out[i,:,:, :2], pred_traj_gt.numpy())
            ###kde_ll.append(nll)
           ### t_kde_ll.append(timestep_nll)
            #print("knll: ", knll)
            
            d.append(diffmin) # contiene el minimo de cada trayectoria
        l2dis += d # contiene la lista de minimos (trayectorias) concatenados para cada batch
        
        # Calculate NLL version 2
        nll2, t_nll2 = compute_kde_nll2(pred_out, batch["pred_traj"].numpy())
        kde_ll2.append(nll2)
        kde_ll_per_step.append(t_nll2)

    ade = [t for o in l2dis for t in o] # average displacement
    fde = [o[-1] for o in l2dis] # final displacement
    #last_kde_nll = [o[-1] for o in t_kde_ll] # final displacement
    return { "mADE": np.mean(ade), "mFDE": np.mean(fde), "kde_nll": np.mean(kde_ll2), "kde_ll_per_step": np.mean(kde_ll_per_step,axis=0)}


def compute_kde_nll2(predicted_trajs, gt_traj): # (256, 50, 12, 2)   (256,12, 2) # 50 posibles predichas se compara con el original
#    print("predicted_trajs.shape: ", predicted_trajs.shape)
    ...
    for batch_num in range(num_batches):
        for timestep in range(num_timesteps):
            try:
                ...
                kde = gaussian_kde(predicted_trajs[batch_num, :, timestep, :].T)
#                print("predicted_trajs[batch_num, :, timestep].T.shape: ", predicted_trajs[batch_num, :, timestep, :].T.shape)
                
                pdf = np.clip(kde.logpdf(gt_traj[batch_num, timestep, :].T), a_min=log_pdf_lower_bound, a_max=None)[0]
#                print("gt_traj[batch_num, timestep, :].T.shape: ", gt_traj[batch_num, timestep, :].T.shape)
#                print("pdf: ", pdf)
